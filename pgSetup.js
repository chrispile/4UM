...
var pgSetup = new function() {
    ...
    this.connect = function() {
        ...
        pgClient.query("CREATE TABLE IF NOT EXISTS Posts(pid serial, username varchar(64) REFERENCES Users(username) ON DELETE CASCADE, sname varchar(100) REFERENCES SUB4UMS(sname) ON DELETE CASCADE, title varchar(300), text varchar(40000), url varchar(200), score integer DEFAULT 0,timestamp timestamp default current_timestamp, PRIMARY KEY(pid))");
        pgClient.query("CREATE TABLE IF NOT EXISTS Subscribes(uid integer REFERENCES Users(uid) ON DELETE CASCADE, sid integer REFERENCES SUB4UMS(sid) ON DELETE CASCADE, sname varchar(100) REFERENCES SUB4UMS(sname) ON DELETE CASCADE, PRIMARY KEY(uid, sid))");
        ...
        pgClient.query("CREATE TABLE IF NOT EXISTS Comments(cid serial, uid integer REFERENCES Users(uid) ON DELETE CASCADE, pid integer REFERENCES Posts(pid) ON DELETE CASCADE, text varchar(1000), PRIMARY KEY(cid, uid, pid))");
